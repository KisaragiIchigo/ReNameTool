...
from PySide6.QtWidgets import (
    ...
    QCheckBox, QSpinBox, QAbstractSpinBox, QFrame, QTableWidgetItem,
    QSizePolicy
)

from style import build_qss, GAP_DEFAULT, PADDING_CARD, RESIZE_MARGIN
from widgets import RenameTable, HistoryCombo
...
# 設定ファイル名
CFG_FILE = "[config]ReNameTool_config.json"
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.setMinimumSize(QSize(50, 50))  # 最小 50x50
        self.resize(1100, 720)

        # アイコン
        ico_path = resource_path("rename.ico")
        self.setWindowIcon(QIcon(ico_path) if os.path.exists(ico_path) else QIcon())
        ...
        # 背景/ガラスカード
        self.bg = QWidget()
        self.bg.setObjectName("bgRoot")
        outer.addWidget(self.bg)

        self.card = QWidget()
        self.card.setObjectName("glassRoot")

        lay = QVBoxLayout(self.bg)
        lay.setContentsMargins(GAP_DEFAULT, GAP_DEFAULT, GAP_DEFAULT, GAP_DEFAULT)
        ...
        self.lbl_title = QLabel("リネームツール")
        self.lbl_title.setObjectName("titleLabel")

        # READMEボタン
        self.btn_readme_top = QPushButton("ReadMe")
        self.btn_readme_top.setObjectName("readmeBtn")
        self.btn_readme_top.setFixedHeight(28)
        self.btn_readme_top.clicked.connect(self._open_readme)

        # タイトルバー小ボタン：最小化/最大化/閉じる
        ...
        bar.addStretch()
        bar.addWidget(self.btn_readme_top)  # ← READMEは最小化の左
        ...
        # ===== 本体 2カラム =====
        body = QHBoxLayout()
        body.setSpacing(GAP_DEFAULT)
        main.addLayout(body, 1)

        # ---------- 左ペイン ----------
        left = QWidget()
        left.setProperty("class", "DarkPanel")
        left.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Expanding)
        left.setMinimumWidth(LEFT_FIXED_WIDTH)
        left.setMaximumWidth(LEFT_FIXED_WIDTH)
        ...
        self.combo_scope.addItems([TARGET_FILES, TARGET_FOLDERS])
        ...
        self.method.addItems([
            "リネーム（置換）", "エリア文字削除", "連番", "日付",
            "フォルダ名追加", "文字列追加", "特定文字の移動/追加"
        ])
        llay.addWidget(self.method)

        # --- 置換 ---
        self.panel_replace = QWidget()
        rp = QVBoxLayout(self.panel_replace); rp.setSpacing(4)
        rp.addWidget(self._label("置換（対象 → 置換後）", "SectionTitle"))
        self.ed_target = HistoryCombo(placeholder="対象1")
        self.ed_repl   = HistoryCombo(placeholder="→ の値")
        row1 = QHBoxLayout()
        row1.addWidget(self._field("対象1", self.ed_target))
        row1.addWidget(self._field("→", self.ed_repl))
        rp.addLayout(row1)

        self.cb_second = QCheckBox("第2置換を有効化")
        rp.addWidget(self.cb_second)

        self.ed_target2 = HistoryCombo(placeholder="対象2")
        self.ed_repl2   = HistoryCombo(placeholder="→ の値")
        row2 = QHBoxLayout()
        row2.addWidget(self._field("対象2", self.ed_target2))
        row2.addWidget(self._field("→", self.ed_repl2))
        rp.addLayout(row2)
        llay.addWidget(self.panel_replace)

        # --- エリア文字削除 ---
        self.panel_sur = QWidget()
        sp = QVBoxLayout(self.panel_sur); sp.setSpacing(4)
        sp.addWidget(self._label("エリア文字削除（開始 / 終了）", "SectionTitle"))
        self.ed_sur_start = HistoryCombo(placeholder="開始")
        self.ed_sur_end   = HistoryCombo(placeholder="終了")
        row3 = QHBoxLayout()
        row3.addWidget(self._field("開始", self.ed_sur_start))
        row3.addWidget(self._field("終了", self.ed_sur_end))
        sp.addLayout(row3)
        llay.addWidget(self.panel_sur)

        # --- 連番 ---
        self.panel_seq = QWidget()
        qp = QVBoxLayout(self.panel_seq); qp.setSpacing(4)
        ...
        row4a = QHBoxLayout()
        row4a.addWidget(self._field("桁数", self.spin_digits))
        row4a.addWidget(self.btn_digits_minus)
        row4a.addWidget(self.btn_digits_plus)
        row4a.addStretch()
        row4b = QHBoxLayout()
        row4b.addWidget(self._field("モード", self.combo_seq_mode))
        row4b.addWidget(self.cb_seq_per_folder)
        row4b.addWidget(self._field("開始番号", self.spin_seq_start))
        row4b.addStretch()
        qp.addLayout(row4a); qp.addLayout(row4b)
        llay.addWidget(self.panel_seq)

        # --- 日付 ---
        self.panel_date = QWidget()
        dp = QVBoxLayout(self.panel_date); dp.setSpacing(4)
        ...
        row5 = QHBoxLayout()
        row5.addWidget(self._field("タイプ", self.combo_date_type))
        row5.addWidget(self._field("モード", self.combo_date_mode))
        dp.addLayout(row5)
        llay.addWidget(self.panel_date)

        # --- フォルダ名追加 ---
        self.panel_folder = QWidget()
        fp = QVBoxLayout(self.panel_folder); fp.setSpacing(4)
        ...
        row6 = QHBoxLayout()
        row6.addWidget(self._field("位置", self.combo_folder_pos))
        row6.addWidget(self.cb_include_parent)
        fp.addLayout(row6)
        llay.addWidget(self.panel_folder)

        # --- 文字列追加 ---
        self.panel_text = QWidget()
        tp = QVBoxLayout(self.panel_text); tp.setSpacing(4)
        tp.addWidget(self._label("文字列追加", "SectionTitle"))
        self.ed_add_text = HistoryCombo(placeholder="追加する文字列")
        self.combo_text_pos = QComboBox(); self.combo_text_pos.addItems(["先頭に追加","末尾に追加"])
        row7 = QHBoxLayout()
        row7.addWidget(self._field("文字列", self.ed_add_text))
        row7.addWidget(self._field("位置", self.combo_text_pos))
        tp.addLayout(row7)
        llay.addWidget(self.panel_text)

        # --- 特定文字の移動/追加 ---
        self.panel_move = QWidget()
        mv = QVBoxLayout(self.panel_move); mv.setSpacing(4)
        mv.addWidget(self._label("特定文字の移動/追加", "SectionTitle"))
        self.ed_move_find = HistoryCombo(placeholder="検索語 / 正規表現")
        self.combo_move_action = QComboBox(); self.combo_move_action.addItems(["元の文字列を削除して移動","元の文字列はそのままで新たに追加"])
        self.combo_move_pos = QComboBox(); self.combo_move_pos.addItems(["先頭に追加","末尾に追加","指定文字の後に追加"])
        self.cb_move_use_find = QCheckBox("検索した文字列を追加に使う"); self.cb_move_use_find.setChecked(True)
        self.ed_move_custom = HistoryCombo(placeholder="自由入力（OFF時に使用）"); self.ed_move_custom.setEnabled(False)
        ...
        self.ed_move_anchor = HistoryCombo(placeholder="アンカー（指定文字）")
        self.cb_move_anchor_regex = QCheckBox("アンカー 正規表現")
        rowm1 = QHBoxLayout(); rowm1.addWidget(self._field("検索語/正規表現", self.ed_move_find))
        rowm2 = QHBoxLayout(); rowm2.addWidget(self._field("アクション", self.combo_move_action)); rowm2.addWidget(self._field("位置", self.combo_move_pos))
        rowm3 = QHBoxLayout(); rowm3.addWidget(self.cb_move_use_find); rowm3.addWidget(self._field("自由入力（OFF時）", self.ed_move_custom))
        rowm4 = QHBoxLayout(); rowm4.addWidget(self.cb_move_delete_all); rowm4.addWidget(self._field("区切り自動付与", self.combo_move_sep))
        rowm5 = QHBoxLayout(); rowm5.addWidget(self.cb_move_regex); rowm5.addWidget(self._field("アンカー（指定文字）", self.ed_move_anchor)); rowm5.addWidget(self.cb_move_anchor_regex)
        ...
        # ---------- 右ペイン ----------
        right = QWidget()
        rlay = QVBoxLayout(right); rlay.setSpacing(6)
        cap = QHBoxLayout()
        ...
        self._sync_maximize_style()

        # ===== シグナル =====
        self.btn_preview.clicked.connect(self._on_click_preview)
    ...
    def _label(self, text: str, cls: str | None = None):
        lb = QLabel(text)
        if cls:
            lb.setProperty("class", cls)
        return lb

    def _field(self, title: str, widget):
        from PySide6.QtWidgets import QVBoxLayout, QWidget, QLabel
        box = QVBoxLayout()
        lb = QLabel(title); lb.setProperty("class", "SectionTitle")
        box.addWidget(lb)
    ...
    def _gather_settings(self) -> Settings:
        return Settings(
            ...
            target=self.ed_target.currentText(),
            replacement=self.ed_repl.currentText(),
            rename_second_active=self.cb_second.isChecked(),
            target_second=self.ed_target2.currentText(),
            replacement_second=self.ed_repl2.currentText(),
            surrounded_start=self.ed_sur_start.currentText(),
            surrounded_end=self.ed_sur_end.currentText(),
            ...
            add_text=self.ed_add_text.currentText(),
            ...
            # move 拡張
            move_find=self.ed_move_find.currentText(),
            ...
            move_custom=self.ed_move_custom.currentText(),
            ...
            move_anchor=self.ed_move_anchor.currentText(),
            move_anchor_regex=self.cb_move_anchor_regex.isChecked(),
        )

    # ====== 履歴の保存/復元 ======
    def _history_widgets(self) -> list[tuple[str, HistoryCombo]]:
        return [
            ("hist_target", self.ed_target),
            ("hist_repl", self.ed_repl),
            ("hist_target2", self.ed_target2),
            ("hist_repl2", self.ed_repl2),
            ("hist_sur_start", self.ed_sur_start),
            ("hist_sur_end", self.ed_sur_end),
            ("hist_add_text", self.ed_add_text),
            ("hist_move_find", self.ed_move_find),
            ("hist_move_custom", self.ed_move_custom),
            ("hist_move_anchor", self.ed_move_anchor),
        ]

    def _remember_all_histories(self):
        for _, w in self._history_widgets():
            w.remember_current()

    def _export_histories(self) -> dict:
        data = {}
        for key, w in self._history_widgets():
            data[key] = w.get_history()
        return data

    def _import_histories(self, data: dict):
        if not data:
            return
        for key, w in self._history_widgets():
            items = data.get(key, [])
    ...
    def _update_panels(self):
        ...
        if not enable_ext:
            self.cb_ext.setChecked(False)

    # ====== プレビュー/実行（履歴記録をフック） ======
    def _on_click_preview(self):
        self._remember_all_histories()  # #説明: プレビュー時に履歴を記録
        self._save_settings()
        self._do_preview()

    def _on_click_run(self):
        self._remember_all_histories()  # #説明: 実行時に履歴を記録
        self._save_settings()
        self._do_run()
    ...
    def _do_preview(self):
        try:
            ...
            self._apply_selection_filter()
            if not items:
                self._toast("変化なし（プレビュー空）")
        except Exception as e:
            save_error_log("preview", str(e))
            self._toast(f"プレビュー中にエラー: {e}")

    def _do_run(self):
        try:
            checked_paths_ordered = self._checked_paths_in_visual_order()
            if not checked_paths_ordered:
                self._toast("チェックされた行がありません。")
                return
            ...
            for r in rows_for_checked:
                ...
                if newp:
                    items.append(RenameItem(old_path=oldp, new_path=newp))
                else:
                    missing_calc_paths.append(oldp)
            ...
            save_error_log("run", str(e))
            self._toast(f"実行中にエラー: {e}")

    def _roll_forward_rows(self, results: list[dict]):
        # #説明: 成功した行は現状を次ステップへロールフォワード
    ...
    def _on_header_clicked(self, col: int):
        ...
        from PySide6.QtWidgets import QWidget as _QW, QHBoxLayout as _QHL, QCheckBox as _QCB, QTableWidgetItem
        for row in snap:
            ...
            for c, cell in enumerate(row["cells"]):
                if c == self.table.COL_SELECT:
                    w = _QW()
                    lay = _QHL(w); lay.setContentsMargins(0,0,0,0); lay.setAlignment(Qt.AlignCenter)
    ...
    def _save_settings(self):
        ...
        data.update(self._export_histories())  # 履歴も保存
        self.cfg.save(data)
    ...
    def _restore_settings(self):
        ...
        try:
            ...
            self.ed_target.setCurrentText(data.get("target","")); self.ed_repl.setCurrentText(data.get("replacement",""))
            self.cb_second.setChecked(data.get("rename_second_active", False))
            self.ed_target2.setCurrentText(data.get("target_second","")); self.ed_repl2.setCurrentText(data.get("replacement_second",""))
            self.ed_sur_start.setCurrentText(data.get("surrounded_start","")); self.ed_sur_end.setCurrentText(data.get("surrounded_end",""))
            ...
            self.ed_add_text.setCurrentText(data.get("add_text",""))
            ...
            # 履歴復元（先に履歴セット→現在値は上記で反映済み）
            self._import_histories(data)

            # move拡張
            self.ed_move_find.setCurrentText(data.get("move_find",""))
            ...
            self.ed_move_custom.setCurrentText(data.get("move_custom",""))
            ...
            self.ed_move_anchor.setCurrentText(data.get("move_anchor",""))
    ...
    def _restore_window_and_header(self):
        ...
        if gb64:
            try:
                g = base64.b64decode(gb64.encode("utf-8"))
        ...
        if hb64:
            try:
                h = base64.b64decode(hb64.encode("utf-8"))
    ...
    def _save_window_and_header(self):
        data = self.cfg.load() or {}
        try:
            g = bytes(self.saveGeometry())
            data[CFG_KEY_GEOM] = base64.b64encode(g).decode("utf-8")
            h = bytes(self.table.horizontalHeader().saveState())
            data[CFG_KEY_HDR]  = base64.b64encode(h).decode("utf-8")
            data.update(self._gather_settings().to_dict())
            data[CFG_KEY_SCOPE] = self._current_scope()
            data.update(self._export_histories())
            self.cfg.save(data)
        except Exception as e:
        ...
        if obj in (self.card, self.bg):
            ...
            # マウスムーブ
                ...
                if self._moving and (e.buttons() & Qt.LeftButton) and not self.isMaximized():
                    ...
                    return True
                local_pos = obj.mapFromGlobal(e.globalPosition().toPoint())
                self._update_cursor(self._edge_at_obj(obj, local_pos))
                return False
            # マウスアップ
    ...
    def _sync_maximize_style(self):
        self.card.setProperty("isMaximized", self.isMaximized())
